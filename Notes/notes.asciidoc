...
Use `git whatchanged` to see recent changes?  e.g. what recent themes were added to oh-my-posh: `git -C ~/.oh-my-posh whatchanged --diff-filter=A -- themes`
...
https://zsh.sourceforge.io/Doc/Release/Expansion.html#Modifiers-1[Chater 14 Expansion] from the https://zsh.sourceforge.io/Doc/Release/index.html[_The Z Shell Manual_]
and https://zsh.sourceforge.io/Guide/zshguide05.html[Chapter 5: Substitutions] from https://zsh.sourceforge.io/Guide/zshguide.html[A User's Guide to the Z-Shell]

.modifiers
[%autowidth]
|===
|Single Letter| Explanation

| a
| absolute path

| A
| absolute path and then apply `realpath`

| p
| print the command but do not execute

| P
| realpath but remove `.` and `..`
...
| x
| like `q` but use whitespace

| w
| make the following modifier work on each word in a string

| f
| repeats the following modifier until the resulting word does not change

| F:expr:
| like `f` but repeats only `n` times if `expr` evalues to `n`

| t [ digits ]
| tail, remove leading pathname components

| W:sep:
| like `w` but `sep` is the separator
|===
...
See also `man zshexpn` (https://man.x-cmd.com/zsh/man1/zshexpn[here] and https://zsh.sourceforge.io/Guide/zshguide05.html[ZSH Chapter 5]) for info about removing quotes. e.g. `${(Q)FOO}` removes one level of quotes.
...
Sometimes I'm on a host and control+r does not work. I can often fix that with `bindkey -e`. Even though I use vim, my fingers remember the normal emacs bindings.

==== default value

`VAR=${VAR:-defaultValue}`

From https://unix.stackexchange.com/a/649719/30160[SE]
...
| `ZSH_ARGZERO`
| The name of the currently running script or the name of the shell
...
I don't use an IDE. I miss the feature that automatically runs a Python program and show its output.
I can mimic this in vim by using `autocmd` and opening the output file in a separate window.
...
==== Convert a .iso to a bootable USB

Inspired by https://discussions.apple.com/thread/256163060[The best way to burn iso to usb on MAC for windows iso?]

.`dd` and `diskutil` can create a bootable USB disk
[CODE]
-----
diskutil image create from --format UDRW ubuntu-24.04.3-live-server-amd64.iso ubuntu-server-24.img
diskutil list # find the USB drive, it was /dev/disk4, also look in /Volumes/
sudo dd if=ubuntu-server-24.img of=/dev/rdisk4 bs=1m # next time maybe try `iflag=direct oflag=direct`?
...
https://stackoverflow.com/a/77491061/1698426[TIL pip-system-certs]
...
I like Ubuntu "minimal". Do not `unminimize` though, even when prompted to do that. `/usr/bin/unminimize` will install a lot, even snap. Instead, do this to make a 'minimized' Ubuntu installation useful:

. nuke `/etc/dpkg/dpkg.cfd.d/excludes`
...
. `rm -f /usr/bin/man; dpkg-divert --quiet --remove --rename /usr/bin/man` (taken from `/usr/bin/unminimize`)

If a man page is not available (e.g. `man tar` does not show the man page for `tar`), try `sudo apt install --reinstall tar`.

=== locale

A minimized Ubuntu complained about locale. The locale `en_US.UTF-8` might not be installed, look for an error on login about `$LANG` and `$LC_*`.

.prevent warning messages about `en_US.UTF-8` being absent
[CODE]
-----
sudo apt update
...
==== still more Ubuntu updates to disable

Next time, when install a fresh Ubuntu host, consider also:
...
See `/etc/apt/apt.conf.d` for more possibilities.

Also try `systemctl list-unit-files --all` to see what services have the word 'update' or 'apt' or 'daily' or 'timer' in them.
...
I saw an error trying to install `ntp` with `apt`. Remove `systemd-timesyncd`:
...
. `sudo apt remove --purge lxd-installer`. Snap what?
. On a VM, I usually want to install `parted` and `open-vm-tools`.
...
* https://news.ycombinator.com/item?id=13349211[dd is not a disk writing tool]
** i.e. for writing to USB disks, use `iflag=direct oflag=direct`
